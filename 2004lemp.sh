...
set -e
echo '----------------------------------'
echo ' Install scripts for Ubuntu 20.04 '
echo '----------------------------------'
...
case $opt in
    ...
    3)
        ...
        sed -r -i "s/;?cgi.fix_pathinfo\s*=\s*[0-9]+/cgi.fix_pathinfo=0/" /etc/php/7.4/fpm/php.ini
        sed -r -i "s/;?post_max_size\s*=\s*\S+/post_max_size=$postsize/" /etc/php/7.4/fpm/php.ini
        sed -r -i "s/;?upload_max_filesize\s*=\s*\S+/upload_max_filesize=$postsize/" /etc/php/7.4/fpm/php.ini
        sed -r -i "s/;?date.timezone\s*=\s*\S*/date.timezone=$timezone/" /etc/php/7.4/fpm/php.ini
        sed -r -i "s/;?expose_php\s*=\s*\S+/expose_php=Off/" /etc/php/7.4/fpm/php.ini
    ...
    4)
        ...
        sed -r -i "s/user\s*=\s*\S*/user = $webuser/" /etc/php/7.4/fpm/pool.d/www.conf
        sed -r -i "s/listen\.owner\s*=\s*\S*/listen.owner = $webuser/" /etc/php/7.4/fpm/pool.d/www.conf
        rm /var/log/nginx/*
    ...
    5)
        ...
        echo "                fastcgi_pass unix:/var/run/php/php7.4-fpm.sock;" >> /etc/nginx/sites-available/$sitename
    ...
    7)
        ...
        echo "    if 5 restarts within 5 cycles then timeout" >> /etc/monit/conf.d/lemp-services
        echo "" >> /etc/monit/conf.d/lemp-services
        echo "check process php7.4-fpm with pidfile /run/php/php7.4-fpm.pid" >> /etc/monit/conf.d/lemp-services
        echo "    start program = \"/etc/init.d/php7.4-fpm start\"" >> /etc/monit/conf.d/lemp-services
        echo "    stop program = \"/etc/init.d/php7.4-fpm stop\"" >> /etc/monit/conf.d/lemp-services
        echo "    if failed unixsocket /run/php/php7.4-fpm.sock then restart" >> /etc/monit/conf.d/lemp-services
        echo "    if 5 restarts within 5 cycles then timeout" >> /etc/monit/conf.d/lemp-services
